...
// Frontend pages still call the legacy "/contracts" API while the backend now serves
// the module under "/agreements". Rewrite those requests centrally so older pages keep
// working without touching every caller.
const normalizeLegacyAgreementUrl = (url?: string) => {
    if (!url) return url;

    let normalized = url;
    if (normalized === '/contracts' || normalized.startsWith('/contracts/') || normalized.startsWith('/contracts?')) {
        normalized = `/agreements${normalized.slice('/contracts'.length)}`;
    }

    return normalized.replace(/\/risks(\/|$)/g, '/findings$1');
};

// Request interceptor
apiClient.interceptors.request.use(
    (config) => {
        const normalizedUrl = normalizeLegacyAgreementUrl(config.url);
        if (normalizedUrl !== config.url) {
            console.info(`[API Rewrite] ${config.url} -> ${normalizedUrl}`);
