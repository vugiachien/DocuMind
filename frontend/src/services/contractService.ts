...
const normalizeContractPayload = (data: ContractCreate) => {
    const agreementTypeId = data.agreementTypeId || data.contractTypeId;
    const auditPolicyId = data.auditPolicyId || data.playbookId;
...
const normalizePlaybookSummary = (playbook: any) => ({
    ...playbook,
...
const contractService = {
    ...
    async getContract(id: string): Promise<Contract> {
        ...
        // Backend returns "findings" but frontend expects "risks"
        if (data.findings && !data.risks) {
            data.risks = data.findings;
        }
        return data;
    },
    ...
    async createContract(data: ContractCreate): Promise<Contract> {
        const response = await apiClient.post('/contracts/', normalizeContractPayload(data));
        return response.data;
    },

    // Update contract
    async updateContract(id: string, data: ContractCreate): Promise<Contract> {
        const response = await apiClient.put(`/contracts/${id}`, normalizeContractPayload(data));
        return response.data;
    },
    ...
    async getPlaybooks(type?: FrontendPlaybookType): Promise<any[]> {
        const params = type ? { type: toBackendPlaybookType(type) } : {};
        const response = await apiClient.get('/audit_policies/', { params });
        return response.data.map(normalizePlaybookSummary);
    },
