...
const ContractTypePage: React.FC<{ embedded?: boolean }> = ({ embedded = false }) => {
    ...
    const columns = [
        {
            ...
            key: 'code',
            width: 100,
            render: (text: string) => <Tag color="#f8f9fa" style={{ color: '#667085', border: '1px solid rgba(198,198,205,0.4)', borderRadius: '4px' }}>{text}</Tag>,
        },
        {
            title: 'Name',
            dataIndex: 'name',
            key: 'name',
            width: 300,
            render: (text: string) => <span className="contract-type-name" style={{ color: '#24303a' }}>{text}</span>,
        },
        {
            title: 'Description',
            ...
            render: (text: string, record: ContractType) => {
                if (!text) return <Text style={{ color: '#8c8c8c' }}>—</Text>;
                ...
                <Space>
                    {record.templateUrl ? (
                        ...
                        <Button
                            ...
                            icon={<UploadOutlined />}
                            loading={uploadingTemplateId === record.id}
                            style={{ color: '#667085', border: '1px solid rgba(198,198,205,0.4)', borderRadius: '4px' }}
                            onClick={() => {
                                const input = document.createElement('input');
                                input.type = 'file';
                                input.accept = '.docx';
                        ...
                        <Text style={{ color: '#8c8c8c', fontSize: '13px' }} className="template-no-text">No template</Text>
                    )}
                </Space>
            ),
        },
    ...
    return (
        <Card bordered={!embedded} className="contract-type-card" style={embedded ? { boxShadow: 'none', padding: 0, border: 'none' } : {}}>
            <div className="contract-type-header">
                <h2 className="contract-type-title" style={{ fontFamily: 'Inter, sans-serif', fontSize: '24px', fontWeight: 500, margin: 0, color: '#24303a' }}>Contract Types</h2>
                {isAdmin && (
                    <Button
                        type="primary"
                        icon={<PlusOutlined />}
