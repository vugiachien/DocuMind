...
import re
import logging
from app.modules.agreements.modifier import contract_modifier
...
class ContractService:
    ...
    def analyze_contract(self, db: Session, contract_id: str, user: models.User, background_tasks, full_context_mode: bool = False) -> models.Agreement:
        ...
        # Enqueue task in Celery so heavy analysis does not block the API process.
        from app.worker import process_upload_task
        task = process_upload_task.delay(current_ver.id, full_context_mode)

        logger.info(
            "✅ Enqueued analysis task in Celery (task_id=%s, full_context_mode=%s)",
            task.id,
            full_context_mode,
        )
