...
@celery_app.task(
    acks_late=True,
    time_limit=TaskConfig.HARD_TIME_LIMIT,
        ...
        try:
            with db.begin_nested():  # Savepoint
                ...
                for idx, section in enumerate(analysis_result.get("sections", [])):
                    ...
                    section_display = f"{sid} / {stitle}" if stitle else sid
...
def convert_pdf_task(contract_id: str, pdf_file_url: str):
    ...
    try:
        ...
        from app.modules.agreements.service import contract_service
...
def analyze_playbook_task(playbook_id: str):
    ...
    db = SessionLocal()
    try:
        from app.modules.audit_policies.extractor import playbook_extractor
