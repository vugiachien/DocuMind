...
def _download_contract(url: str) -> Path:
    ...
    # Monolith mode: storage_service returns relative paths like /api/downloads/...
    # In this case, read directly from local storage instead of making HTTP request
    if url.startswith("/api/downloads/"):
        object_name = url.replace("/api/downloads/", "", 1)
        from app.services.storage_service import storage_service
        file_content = storage_service.download_file(object_name)
        filename_hint = Path(object_name).name or "agreement.docx"
        with tempfile.NamedTemporaryFile(delete=False, dir=dest, suffix=Path(filename_hint).suffix or ".docx") as tmp:
            tmp.write(file_content)
            temp_path = Path(tmp.name)
        return temp_path

    # Standard HTTP download for remote URLs
